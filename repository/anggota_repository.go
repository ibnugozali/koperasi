...
func GetPendingAnggota() ([]models.Anggota, error) {
	...
	rows, err := db.Query("SELECT id_anggota, nama_anggota, username, no_telepon, tgl_gabung, unit_kerja, fakultas_code, COALESCE(fakultas, '') FROM anggota WHERE status = 'pending, pending_bendahara' ORDER BY LPAD(nomor_urut, 4, '0') DESC")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
